...
public class DAOLibros extends AbstractDAO {
        ...
        if (rsLibro.next())
        {
            ...
            try {
                ...
                while (rsEjemplares.next()){
                    try{
                        stmPrestamos = con.prepareStatement("select p.usuario, p.fecha_prestamo, p.fecha_vencimiento "
                            + "from prestamo p "
                            + "where p.ejemplar = ? "
                            + "and p.libro = ? "
                                + "and p.fecha_devolucion is null");
                        stmPrestamos.setInt(1, rsEjemplares.getInt("num_ejemplar"));
                        stmPrestamos.setInt(2, resultado.getIdLibro());
                        ...
                        if (rsPrestamos.next()){
                            resultado.addEjemplar(new Ejemplar(resultado.getIdLibro(), rsEjemplares.getInt("num_ejemplar"),
                                                              ...
                                                              rsEjemplares.getString("ano_compra"), 
                                                                rsPrestamos.getString("usuario"),
                                                                LocalDate.parse(rsPrestamos.getString("fecha_prestamo")),
                                                                LocalDate.parse(rsPrestamos.getString("fecha_vencimiento"))));
                        }else{
                            resultado.addEjemplar(new Ejemplar(resultado.getIdLibro(), rsEjemplares.getInt("num_ejemplar"),
                                                          rsEjemplares.getString("localizador"),
                                                          rsEjemplares.getString("ano_compra"), 
    ...
    public java.util.List<Ejemplar> consultarEjemplaresLibro(Integer idLibro){
        ...
        while (rsEjemplares.next())
        {
             try{
                        ...
                        rsPrestamos = stmPrestamos.executeQuery();
                        if(rsPrestamos.next()){
                            resultado.add(new Ejemplar(idLibro, rsEjemplares.getInt("num_ejemplar"),
                                                          rsEjemplares.getString("localizador"),
                                                          rsEjemplares.getString("ano_compra"), 
                                                            rsPrestamos.getString("usuario"),
                                                            LocalDate.parse(rsPrestamos.getString("fecha_prestamo")),
    ...
    public void borrarLibro(Integer idLibro){
        ...
        PreparedStatement stmLibro=null;
        PreparedStatement stmEjemplares = null;
        ResultSet rsEjemplares = null;

        con=super.getConexion();

        try {
            stmEjemplares = con.prepareStatement("select * from ejemplar where libro = ?");
            stmLibro=con.prepareStatement("delete from libro where id_libro = ?");
            stmLibro.setInt(1, idLibro);
            stmEjemplares.setInt(1, idLibro);
            rsEjemplares=stmEjemplares.executeQuery();
            if (rsEjemplares.next()){
                this.getFachadaAplicacion().muestraExcepcion("No se puede eliminar un libro con ejemplares.");
            }else{
                stmLibro.executeUpdate();
            }
          ...
          try {stmEjemplares.close(); if (stmLibro != null) stmLibro.close();} catch (SQLException e){System.out.println("Imposible cerrar cursores");}
        }
    }
  ...
  public void borrarEjemplaresLibro(Integer idLibro, java.util.List<Integer> numsEjemplar){
        ...
        try{
            stmPrestamos=con.prepareStatement("select * from prestamo where libro= ? and ejemplar=?");
            ...
            for (Integer i: numsEjemplar){
                ...
                stmPrestamos.setInt(2, i);
                rsPrestamos = stmPrestamos.executeQuery();
                if(rsPrestamos.next()){
                    this.getFachadaAplicacion().muestraExcepcion("No se puede eliminar un ejemplar con préstamos");
                }else{
                    stmEjemplar.executeUpdate();
                }
            }
            ...
            try {if (stmEjemplar != null) stmEjemplar.close(); stmPrestamos.close();} catch (SQLException e){System.out.println("Imposible cerrar cursores");}
        }
    }
