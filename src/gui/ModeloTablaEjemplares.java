...
public class ModeloTablaEjemplares extends AbstractTableModel{
    ...
    public boolean isCellEditable(int row, int col){
       return col>0 && col <3 && getValueAt(row, 3) == null;
    }
    ...
    public Object getValueAt(int row, int col){
        ...
        switch (col){
            ...
            case 3: resultado = ejemplares.get(row).getIdUsuario(); break;
            case 4: 
                if (ejemplares.get(row).getFechaPrestamo() == null)
                    resultado = "";
                else 
                    resultado = ejemplares.get(row).getFechaPrestamo().toString(); break;
            case 5: 
                if (ejemplares.get(row).getFechaVencimiento() == null)
                    resultado = "";
                else 
                    resultado = ejemplares.get(row).getFechaVencimiento().toString(); break;
        }
        return resultado;
    }
